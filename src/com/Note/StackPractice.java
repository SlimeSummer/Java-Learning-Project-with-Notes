...
class Stack {   //模拟栈类
    ...
    void pop(){
        if (this.isEmpty()){
            ...
            data[top-1]=null;  //显式清空栈内元素，以便调试时观察清除动作
            top=top-1;
        }
    }
}
...
/*
总结：
1.在使用if条件语句判断时，涉及到逻辑与&&运算一定要小心，要时刻意识到&&运算是从左到右依次运算的，有短路评估的特性。在
...
方法的优化程度等
2.在pop()方法的data[top-1]=null语句只是为了在调试时便于观察栈内的元素一个一个被删除的过程动作。
3.调试时，“步过”指的是直接运行完这行代码，然后把步骤移动到下一行。“步入”指的是进入到这一行代码的具体执行步骤，而不是一次性跳过
这一行的执行细节
*/